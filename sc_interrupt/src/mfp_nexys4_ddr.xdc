...
#create_clock -period 10.000 -name sys_clk_pin -waveform {0.000 5.000} -add [get_ports CLK100MHZ]  ## qian commented out due to duplicate clock with clk_wiz_0 input clock xdc declaration
create_clock -period 100.000 -name jtag_tck -waveform {0.000 50.000} -add [get_ports {JB[4]}]

# after you create sys_clk_pin on CLK100MHZ and after synthesis/opt keeps the BUFG instance name
# Create a clock for the divided tap cnt16[15] that you feed through BUFG
# 100 MHz / 65536 = ~1.525 kHz (for a 16-bit counter tap)
# After open_run synth_1 (or before impl)
set bufg_o_pin [get_pins sc_interrupt_sys/gpio/sevensegtimer/bufg_cnt16/O]
set segclk_net [get_nets -of_objects $bufg_o_pin]

create_generated_clock -name sevenseg_clk \
  -source [get_clocks sys_clk_pin] \
  -divide_by 65536 \
  $segclk_net

# Slow user outputs: seven-seg anodes/cathodes + LEDs
# (Use your actual port names. Your XDC uses CA..CG and DP, not DISPOUT[*].)
...
set_property -dict {PACKAGE_PIN K1 IOSTANDARD LVCMOS33} [get_ports {JC_1}]
...
set_property -dict { PACKAGE_PIN G6    IOSTANDARD LVCMOS33 } [get_ports { JC_4 }]; #IO_L19P_T3_35 Sch=jc[4]
set_property -dict { PACKAGE_PIN E7    IOSTANDARD LVCMOS33 } [get_ports { JC_7 }]; #IO_L6P_T0_35 Sch=jc[7]
set_property -dict { PACKAGE_PIN J3    IOSTANDARD LVCMOS33 } [get_ports { JC_8 }]; #IO_L22P_T3_35 Sch=jc[8]
set_property -dict { PACKAGE_PIN J4    IOSTANDARD LVCMOS33 } [get_ports { JC_9 }]; #IO_L21P_T3_DQS_35 Sch=jc[9]
set_property -dict { PACKAGE_PIN E6    IOSTANDARD LVCMOS33 } [get_ports { JC_10 }]; #IO_L5P_T0_AD13P_35 Sch=jc[10]
...
set_input_delay  -clock "clk_virt" -min 0.000 [get_ports {JA[3] JB[3]}]
set_input_delay  -clock "clk_virt" -max 20.000 [get_ports {JA[3] JB[3]}]
...
set_output_delay -clock clk_virt -min 0.000  [get_ports {AN[*] LED[*] CA CB CC CD CE CF CG DP JA[*] JB[*] JC_1 JC_10 JC_2 JC_4 JC_7 JC_8 JC_9 JD[*]}]
set_output_delay -clock clk_virt -max 20.000 [get_ports {AN[*] LED[*] CA CB CC CD CE CF CG DP JA[*] JB[*] JC_1 JC_10 JC_2 JC_4 JC_7 JC_8 JC_9 JD[*]}]
